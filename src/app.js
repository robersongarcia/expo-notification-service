...
async function sendNotifications(tokens, data, message) {
  let messages = []
...
const now = new Date()
const nextMonday = new Date(now.getFullYear(), now.getMonth(), now.getDate() + (8 - now.getDay()) % 7, 0, 0, 0)
const timeUntilNextMonday = nextMonday.getTime() - now.getTime()

// run the task once a week (every 604800000 milliseconds)
setInterval(() => sendNotificationsForPatients( 'data', 'Test Patient notification' ), 604800000, timeUntilNextMonday)
